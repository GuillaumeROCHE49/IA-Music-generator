...
def generate_music(positiv_prompt: str, negative_prompt: str, output_path: str,
                   format :str, num_steps: int = 20, seed: int = 42):
...
def create_lirycs(prompt:str, classifier: Classifier) -> str:
    spectrogram = classifier.get_datas()[0]['spectrogram']
    # Create lirycs on the spectrogram using GPT
    message = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are a music expert and a singer."},
            {"role": "user", "content": f"Give me lirycs on a music with this prompt '{prompt}' and this spectrogam:\n\n{spectrogram}"},
        ],
    )
...
if __name__ == "__main__":
    ...
    generate_music(prompt, "blues", "music.wav", "wav")
    classifier = classify_music("music.wav")
    lirycs = create_lirycs(prompt, classifier)
    print(lirycs)